...
func checkForUpdate(api *apiClient) {
	...
	// The applyUpdate function replaces the binary and signals the s
	// process to stop. On Windows service, SCM auto-restart kicks in.
	os.Exit(0)
}
...
func downloadAgent(url string) (string, error) {
	...
	f, err := os.CreateTemp("", "fortrix-agent-update-*.exe")
	if err != nil {
		return "", err
	}
	defer f.Close()
...
// On Windows the current exe is locked while running, so we:
// 1. Copy current exe to .old
// 2. Move new exe into place (same name as current)
// 3. Exit (SCM or watchdog restarts us from the new binary)
func applyUpdate(newExe string) error {
	...
	// Copy current -> backup (for rollback on next start).
	if err := copyFile(current, backup); err != nil {
		return fmt.Errorf("backup: %w", err)
	}

	// On Unix we can rename-over (atomic). On Windows the running exe
	// can usually be renamed but not deleted. MoveFileEx with
	// MOVEFILE_REPLACE_EXISTING works.
	if runtime.GOOS == "windows" {
		if err := moveFileWindows(newExe, current); err != nil {
			os.Remove(backup)
...
// cleanupOldBackup removes the .old backup from a previous update.
